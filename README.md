...
| 类别 | 已实现 |
| --- | --- |
| 模式 | Normal、Insert、Replace、Visual、Visual Line |
...
| 集成 | 每个编辑器独立保存模式；状态栏提示；插入模式沿用 ZeroBrane 原生编辑能力 |
...
也可把文件放进 ZeroBrane Studio 安装目录的 `packages/`，作为该安装实例的插件。完成后重启 IDE；Output 面板出现 `Vim Mode 0.1.0 registered` 即表示加载成功。
...
两种 Visual 模式支持上表中的移动和计数，并支持：

| 键位 | 功能 |
| --- | --- |
| `v` / `V` | 切换字符选择 / 整行选择；再次按当前模式键退出 |
| `o` | 交换选择区两端 |
| `d` 或 `x` | 删除选择区 |
...
- 这是常用 Vim 操作的子集：暂不支持 `.` 重复、文本对象、命名寄存器、宏、标记/跳转、Visual Block、窗口命令、替换命令、vimrc 或自定义映射。
- 搜索是字面文本搜索，没有正则、增量搜索、历史、补全或结果高亮；搜索和 Ex 都使用模态弹窗。
- `y`、`d`、`c`、`x` 等会写入一个内部寄存器，并可向系统剪贴板复制；`p`/`P` 不会主动读取插件外部后来写入的剪贴板内容。
- Normal/Visual 模式会接管可打印字符以及它收到的导航和 `Ctrl` 键，但 ZeroBrane 的全局快捷键会先于编辑器事件执行。2.01 的默认冲突包括 `Ctrl-R`（替换）、`Ctrl-U`（注释）、`Ctrl-F`（查找）、`Ctrl-B`（符号导航）、`Ctrl-Y`（重做）和 `Ctrl-C`（复制）；这些键在未重映射时可能无法触发对应 Vim 操作。macOS 的 `Command` 组合键不由插件接管。
