...
test("dot repeats atomic and counted Normal-mode changes", function()
  local value = editor("one two three four")
  press(value, "d", "w")
  equal(value:GetText(), "two three four")
  press(value, ".")
  equal(value:GetText(), "three four")
  press(value, "2", ".")
  equal(value:GetText(), "")
end)

test("dot repeats Insert text and operator-plus-Insert changes", function()
  local value = editor("abc")
  press(value, "i")
  value:TypeText("X")
  press(value, "<Esc>", "l", ".")
  equal(value:GetText(), "XXabc")

  value = editor("one two")
  press(value, "c", "w")
  value:TypeText("X")
  press(value, "<Esc>", "l", ".")
...
test("named registers append and can be selected for put", function()
  local value = editor("one two three")
  press(value, '"', "a", "d", "w")
  equal(seam.runtime.registers.a.text, "one ")
  press(value, '"', "A", "d", "w")
  equal(seam.runtime.registers.a.text, "one two ")
  equal(seam.runtime.register.text, "two ", "the unnamed register tracks the latest delete")
  press(value, '"', "a", "P")
  equal(value:GetText(), "one two three")
end)

test("numbered, yank, small-delete, and black-hole registers follow Vim rules", function()
  local value = editor("one\ntwo\nthree")
  press(value, "y", "y")
  equal(seam.runtime.registers["0"].text, "one\n")
  press(value, "d", "d")
  equal(seam.runtime.registers["1"].text, "one\n")
  press(value, "d", "d")
  equal(seam.runtime.registers["1"].text, "two\n")
  equal(seam.runtime.registers["2"].text, "one\n")

  value = editor("abc")
  press(value, "x")
  equal(seam.runtime.registers["-"].text, "a")
  local unnamed = seam.runtime.register.text
  press(value, '"', "_", "x")
  equal(value:GetText(), "c")
  equal(seam.runtime.register.text, unnamed, "black-hole delete preserves unnamed register")
end)

test("last-insert, search, command, and register listing are recorded", function()
  local value = editor("abc foo")
  press(value, "i")
  value:TypeText("XY")
  press(value, "<Esc>")
  equal(seam.runtime.registers["."].text, "XY")

  value = editor("foo foo")
  press(value, "*")
  equal(seam.runtime.registers["/"].text, "foo")

  local printed = ""
  _G.ide = {
    GetDocument = function() return nil end,
    Print = function(self, text) printed = text end,
  }
  local state = seam.getstate(value)
  truthy(seam.executeex(value, state, "registers"))
  equal(seam.runtime.registers[":"].text, "registers")
  truthy(printed:find('"%.%s+XY') ~= nil)
end)
...
test("Visual Block I, A, and c replicate inserted text across rows", function()
  local value = editor("ab\ncd")
  press(value, "<C-v>", "j", "I")
  value:TypeText("X")
  press(value, "<Esc>")
  equal(value:GetText(), "Xab\nXcd")

  value = editor("ab\ncd")
  press(value, "<C-v>", "j", "l", "A")
  value:TypeText("!")
  press(value, "<Esc>")
  equal(value:GetText(), "ab!\ncd!")

  value = editor("ab\ncd")
  press(value, "<C-v>", "j", "c")
  value:TypeText("Q")
  press(value, "<Esc>")
  equal(value:GetText(), "Qb\nQd")
end)

test("dot repeats a Visual Block insertion", function()
  local value = editor("a\nb\nc\nd")
  press(value, "<C-v>", "j", "I")
  value:TypeText("X")
  press(value, "<Esc>", "2", "j", ".")
  equal(value:GetText(), "Xa\nXb\nXc\nXd")
end)
...
test("gt, gT, counts, and Ex commands switch editor tabs", function()
  ...
  equal(active, 3)
  press(third, "g", "<Tab>")
  equal(active, 2, "g<Tab> returns to the alternate tab")
  press(second, "g", "t")
  equal(active, 3)
  press(third, "g", "T")
  equal(active, 2)
